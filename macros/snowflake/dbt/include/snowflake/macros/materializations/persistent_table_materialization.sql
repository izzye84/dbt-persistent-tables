...
{% materialization persistent_table, adapter='snowflake' %}
    ...
    {%- set database = ("_").join(model['name'].split("_")[0:2]) -%}
    {%- set schema = ("_").join(model['name'].split("_")[2:3]) -%}
    {%- set identifier = ("_").join(model['name'].split("_")[3:]) -%}
    {{ log("DatabaseName " ~ database ~ "SchemaName " ~ schema ~ "identifier " ~ identifier ~ ".") }}




    {% set current_relation = adapter.get_relation(database=database, schema=schema, identifier=identifier) %} 
    {{ log("current_relation " ~ current_relation ) }} 
    ...
    {%- set backup_relation = api.Relation.create(database=database,
                                                  ...
                                                  type='table') -%}
    {{ log("backup_relation " ~ backup_relation ) }}
...
-- https://github.com/fishtown-analytics/dbt/blob/f9c8442260e48bdd8bb7805b2e7541ab91492bb1/plugins/snowflake/dbt/include/snowflake/macros/adapters.sql
        {{ adapter.expand_target_column_types(from_relation=tmp_relation, to_relation=current_relation) }}
